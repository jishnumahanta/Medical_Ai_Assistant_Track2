...
const getSystemMessageType = (message: string): string => {
  ...
  if (splitpiece.includes('timeline')) {
    return 'Here is your Medical timeline History'
  } else {
...
const predefinedQueries = {
  ...
  last_hospital_visit: 'When was the patient\'s last hospital visit?',
  medications: 'What are the patient\'s medications?',
  recent_encounters: 'What are the patient\'s recent encounters?',
  // Add more pre-defined queries here
};

const sendPredefinedQuery = (queryType) => {
  const query = predefinedQueries[queryType];
  if (query) {
    formState.currentQuery = query;
    sendQuery();
  }
};
...
<template>
  <q-layout view="lHh lpr lFf"  style="height: 736px">
    <q-header elevated class="bg-blue text-center">

      <div class="q-pa-md text-h4 text-center">MAIA-Medical AI Assistant</div>
    </q-header>
  ...
  <div class="chat-area q-pt-xl " id="chat-area">
    <div v-for="(x, idx) in chatHistory" :key="idx">
      ...
      <q-chat-message :name="x.role" v-if="x.role === 'system'" size="8" sent>
        <q-card color="secondary">
          ...
          <q-card-actions>
            <q-btn color="primary"
              label="Click here for view the details"
              @click="
    ...
    <div class="signature-buttons" v-if="chatHistory.length">
      <q-btn size="sm" color="green" label="Save Locally" @click="saveToFile" />
      <q-btn
        size="sm"
        color="green"
        ...
        @click="saveToNosh"
      ></q-btn>
      <q-btn size="sm" color="red" label="End without Saving" @click="closeNoSave" />
    </div>
           

  </div>
  <div class="bottom-toolbar">
  <div class="pre-defined-queries q-pa-md q-gutter-sm flex flex-center " >
  <q-btn
    v-for="(query, queryType) in predefinedQueries"
    :key="queryType"
    size="sm"
    color="primary"
    :label="queryType.replace('_', ' ').toUpperCase()"
    @click="sendPredefinedQuery(queryType)"
  />
</div>
    <div class="prompt">
      <div class="inner">
